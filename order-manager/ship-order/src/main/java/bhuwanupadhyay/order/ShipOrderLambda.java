...
import bhuwanupadhyay.order.model.Order.OrderReadyEvent;
...
public class ShipOrderLambda implements RequestHandler<KinesisEvent, Void> {
    ...
    public Void handleRequest(KinesisEvent kinesisEvent, Context context) {
        RecordDeaggregator.deaggregate(kinesisEvent.getRecords())
                .forEach(record -> toEventPayload(record.getData(), OrderReadyEvent.class)
                        .map(OrderReadyEvent::getOrderId)
                        .ifPresent(orderId ->
                                Result.with(order(orderId), withError("Order does not exist with id: " + orderId))
                                        .onSuccess(Order::shipOrder)
